...
from openpi_on_thor.calibration_data_openarm import load_openarm_calibration_data
...
def _prepare_model_for_export(
    ...
    if precision.lower() == "fp8":
        ...
        if config_obj is not None and checkpoint_dir is not None:
            # Use OpenArm-specific calibration for OpenArm configs
            if "openarm" in config_obj.name.lower():
                print("  Using OpenArm-specific calibration loader...")
                calibration_data = load_openarm_calibration_data(
                    config_obj,
                    checkpoint_dir,
        ...
        # FP16 without quantization - slower but more accurate
        print("  Using FP16 precision (no quantization)")
        dtype_str = "float16"
    else:
        raise ValueError(
            f"Unsupported precision: {precision}. Use 'fp8' for quantization or 'fp16' for baseline."
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="fp8",
        choices=["fp8", "FP8", "fp16", "FP16"],
        help="Model precision type (default: fp8). Use fp16 for debugging/validation.",
    )
    parser.add_argument(
        "--num_calibration_samples",
        type=int,
    ...
    parser.add_argument(
        "--static_shapes",
        action="store_true",
        help="Export with static shapes (required for NVFP4). Disables dynamic batch/sequence length.",
    )
