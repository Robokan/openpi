...
def pi0_tensorrt_sample_actions(self, device, observation, noise=None, num_steps=None):
    ...
    import logging
    logger = logging.getLogger("trt_forward")
    if not hasattr(pi0_tensorrt_sample_actions, '_log_count'):
        pi0_tensorrt_sample_actions._log_count = 0
    pi0_tensorrt_sample_actions._log_count += 1
    
    if pi0_tensorrt_sample_actions._log_count <= 5 or pi0_tensorrt_sample_actions._log_count % 50 == 0:
        a = actions.float()
        logger.warning(f"[TRT] actions shape={actions.shape}, dtype={actions.dtype}, "
                      f"min={a.min().item():.4f}, max={a.max().item():.4f}, "
                      f"mean={a.mean().item():.4f}, has_nan={a.isnan().any().item()}, "
                      f"first_action[:8]={a[0,0,:8].tolist()}")
